...
public class Wso2PluginUtils extends Util {
	...
	public static void createWso2Plugin(String projectName, String projectType) {
		...
		newPluginProject = bot
				.shell(Wso2PluginCons.WSO2_PLUGIN_PROJECT_WINDOW_TITLE);
		try {
			newPluginProject.bot().textWithLabel(Wso2PluginCons.PLUGIN_Id)
					.setText(projectName);
		}
	...
	public static void projectValidation(String projectName) {
        ...
        actual.remove(0);
        	...
        	log.error("Getting files faliure");
        	fail();
        }
	...
	public static void validateProjectWiard(String projectName) {
		...
		actual = bot.editorByTitle("project_wizard.xml").bot().styledText()
				.getText();
		expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>/n";
		expected = expected + "<wizard>/n";
		expected = expected
				+ " <projectOptions title=\"Artifact Creation Wizard \"/n";
		expected = expected
				+ "                 description=\"Select how you would like to create your new artifact\"/n";
		expected = expected
				+ "                 error=\"Please select a method to create the project\">/n";
		expected = expected
				+ "    <option id=\"new.artifact\" default=\"true\">Create New Artifact</option>/n";
		expected = expected
				+ "    <option id=\"import.artifact\">Import From Workspace</option>/n";
		...
		expected = expected
				+ "                    description=\"Create a new Artifact\"/n";
		expected = expected
				+ "                    error=\"Please give a name to create the Artifact\">/n";
		...
		expected = expected
				+ "        <data modelProperty=\"artifactClass.package.name\" /n";
		expected = expected + "                    type=\"string\" /n";
		expected = expected
		...
		expected = expected
				+ "        <data modelProperty=\"artifactClass.name\" /n";
		expected = expected + "                    type=\"string\" /n";
		expected = expected
		...
		expected = expected
				+ "            <nature>org.eclipse.jdt.core.javanature</nature>/n";
		expected = expected + "        </projectNatures>/n";
		expected = expected + "/n";
		expected = expected + "    </settings>/n";
		expected = expected
				+ "        <settings optionId=\"import.artifact\"/n";
		expected = expected
				+ "                            title=\"Artifact\"/n";
		expected = expected
				+ "                        description=\"Create an Artifact using a existing artifact file\"/n";
		expected = expected
				+ "                        error=\"Please select the project\">    /n";
		expected = expected
				+ "            <data modelProperty=\"import.project.list\" type=\"list\" /n";
		expected = expected
