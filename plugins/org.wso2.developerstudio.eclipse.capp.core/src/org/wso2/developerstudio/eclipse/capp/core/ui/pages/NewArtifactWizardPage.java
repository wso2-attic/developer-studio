...
public class NewArtifactWizardPage extends WizardPage implements Observer {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.axis2.tool.service.eclipse.ui.AbstractServiceWizardPage#
	...
	private void validate() {
		...
		if (getArtifactVersion() == null
				|| getArtifactVersion().trim().equals("")) {
			setErrorMessage("The artifact version cannot be blank");
			setPageComplete(false);
			return;
		}

		if (getArtifactServerRole() == null
				|| getArtifactServerRole().trim().equals("")) {
	...
	private void fillServerRoleCombo() {
		...
		cmbArtifactServerRole.remove(0,
				cmbArtifactServerRole.getItemCount() - 1);
		...
		if (getArtifactXmlPath() != null) {
			Display.getCurrent().asyncExec(new Runnable() {
				public void run() {
					...
					ICAppArtifactManager cAppArtifactManager = CAppEnvironment
							.getcAppManager();
					if (cAppArtifactManager.isCAppProject(project)) {
						IServerRole[] serverRoles = cAppArtifactManager
								.getServerRoles(project);
						for (IServerRole serverRole : serverRoles) {
							cmbArtifactServerRole.add(serverRole
									.getServerRoleName());
						}
					}
				}
