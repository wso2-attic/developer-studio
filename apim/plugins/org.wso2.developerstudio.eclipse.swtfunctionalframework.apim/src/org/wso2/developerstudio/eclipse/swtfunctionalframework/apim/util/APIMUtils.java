...
public class APIMUtils extends PerspectiveLoginUtil {
	...
	public static SWTBotTreeItem apimMainTree(String userName, String url) {
		try {
			SWTBotView apimRegistry = Util.bot
	...
	public static void createSequenceAPIM(String userName, String url,
		...
		SWTBotShell createSequenceShell = Util.bot
				.shell(APIMConstants.CREATE_SEQUENCE);
		try {
			createSequenceShell.bot()
					.textWithLabel(APIMConstants.SEQUENCE_NAME)
	...
	public static void renameSequenceAPIM(String userName, String url,
		...
		try {
			...
			SWTBotShell renameShell = Util.bot
					.shell(APIMConstants.RENAME_RESOURCE);
			renameShell.bot().textWithLabel(APIMConstants.NEW_NAME)
					.setText(newName);
	...
	public static void discardAllChanges() {
		Util.bot.toolbarButtonWithTooltip(
	...
	public void login(String userName, String password) {
		try {
			...
			SWTBotShell login = Util.bot
					.shell(APIMConstants.LOGIN_TO_API_MANAGER_REGISTRY);
			login.bot().textWithLabel(APIMConstants.USER_NAME)
					.setText(userName);
			login.bot().textWithLabel(APIMConstants.PASSWORD2)
					.setText(password);
			Util.bot.button(CommonConstants.OK).click();
